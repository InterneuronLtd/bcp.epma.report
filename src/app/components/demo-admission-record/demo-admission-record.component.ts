...
export class DemoAdmissionRecordComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  getRowStyle(allerg: any): { [key: string]: string } {
    if (allerg.clinicalstatusvalue === 'Active' && allerg.criticality == "Life Threatening") {
      return { color: 'red' };
    } else if (allerg.clinicalstatusvalue === 'Resolved') {
      return { color: 'gray' };
    } else {
      return { color: 'black' };
    }
  ...
  ngOnInit() {
    ///////
    this.shownoAllergy =false
    for(let i= 0 ; i< this.appService.allergyintolerance.length ;i++){
      this.appService.allergyintolerance[i].reactiontext = JSON.parse( this.appService.allergyintolerance[i].reactionconcepts)
    }
    let noknownallergies = this.appService.allergyintolerance.find(x=>x.causativeagentdescription.includes( "No known allergies") && x.clinicalstatusvalue == "Active" && x.category == "Flag" )
    if(noknownallergies){
      this.shownoAllergy =true
    }
  ...
  protocolGetDay() {
    for (let key in this.hs.prescriptionDictionary) {
      if (this.hs.prescriptionDictionary[key][0] && this.hs.prescriptionDictionary[key][0].frequency === 'protocol') {
    ...
    if (!this.prescriptionProperFlow[pres.prescription_id]) {
      ...
      for (let n = 0; n < rows; n++) {
        for (let m = 0; m < cols; m++) {
          ...
          if (n >= this.hs.prescriptionDictionary[pres.prescription_id][0] && this.hs.prescriptionDictionary[pres.prescription_id][0].protocolCount - 1 && frequency !== 'protocol' && !time) {
            let dataObj = this.returnTransferTime(n, pres, m);
            time = dataObj.time;
            dose = dataObj.dose;
            administrationstatus = dataObj.administrationstatus;
            flag = false;
          } else if (n >= this.hs.prescriptionDictionary[pres.prescription_id][0] && this.hs.prescriptionDictionary[pres.prescription_id][0].protocolCount && frequency === 'protocol' && !time) {
            let dataObj = this.returnTransferTime(n, pres, m);
            time = dataObj.time;
            dose = dataObj.dose;
            administrationstatus = dataObj.administrationstatus;
      ...
      if (!this.prescriptionProperFlow[pres.prescription_id][i][j].administered) {
        ...
        if (checkTime2 < checkTime && currDate === this.dates[0]) {
          ...
          if(pres.infusiontype_id == "rate")
          {
            ...
            if( this.hs.prescriptionDictionary[pres.prescription_id][0] && this.hs.prescriptionDictionary[pres.prescription_id][i].doctorsorder || this.hs.prescriptionDictionary[pres.prescription_id][i].titration)
            {
              return this.prescriptionProperFlow[pres.prescription_id][i][j].administrationstatus;
            }
            return 'Missed';
